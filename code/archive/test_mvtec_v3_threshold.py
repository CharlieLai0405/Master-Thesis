"""
方案 3: Threshold-based Precision
用 anomaly_map.max() 配合 per-class optimal threshold 判斷正常/異常
取代 LLM 的 Yes/No 文字回答
...
def predict_with_tta(input_text, image_path, normal_img_paths, describles_text):
    """TTA: original + hflip + vflip, average anomaly maps"""
...
p_auc_list = []
i_auc_list = []
precision_llm = []      # Original LLM-based precision
precision_thresh = []    # Threshold-based precision
...
for c_name in CLASS_NAMES:
    ...
    i_pred_arr, i_label_arr = np.array(i_pred), np.array(i_label)

    p_auroc = round(roc_auc_score(p_label.ravel(), p_pred.ravel()) * 100, 2)
    i_auroc = round(roc_auc_score(i_label_arr.ravel(), i_pred_arr.ravel()) * 100, 2)

    # Find optimal threshold using Youden's J statistic
    fpr, tpr, thresholds = roc_curve(i_label_arr, i_pred_arr)
    j_scores = tpr - fpr
    best_idx = np.argmax(j_scores)
    best_threshold = thresholds[best_idx]

    # Threshold-based precision
    thresh_preds = (i_pred_arr >= best_threshold).astype(int)
    right_thresh = np.sum(thresh_preds == i_label_arr)
    total = len(i_label_arr)
    thresh_acc = round(100 * right_thresh / total, 2)

    llm_acc = round(100 * right_llm / (right_llm + wrong_llm), 2)

    p_auc_list.append(p_auroc)
    i_auc_list.append(i_auroc)
    precision_llm.append(llm_acc)
    precision_thresh.append(thresh_acc)

    print(f"{c_name} i_AUROC: {i_auroc}")
    print(f"{c_name} p_AUROC: {p_auroc}")
    print(f"{c_name} precision_LLM: {llm_acc}")
    print(f"{c_name} precision_threshold: {thresh_acc} (threshold={best_threshold:.4f})")

print(f"\ni_AUROC: {torch.tensor(i_auc_list).mean()}")
print(f"p_AUROC: {torch.tensor(p_auc_list).mean()}")
print(f"precision_LLM: {torch.tensor(precision_llm).mean()}")
print(f"precision_threshold: {torch.tensor(precision_thresh).mean()}")
