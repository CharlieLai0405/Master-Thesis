"""
VisA Best Combo + Per-class TTA (skip pcb2/candle)
"""
import os
...
parser.add_argument("--use_prompt_adapter", action="store_true", help="Use improved describles")
...
# Classes where TTA hurts performance - use non-TTA prediction
NO_TTA_CLASSES = {"pcb2", "candle"}
...
# Original (baseline) describles
describles_original = {}
describles_original['candle'] = "This is a photo of 4 candles for anomaly detection, every candle should be round, without any damage, flaw, defect, scratch, hole or broken part."
describles_original['capsules'] = "This is a photo of many small capsules for anomaly detection, every capsule is green, should be without any damage, flaw, defect, scratch, hole or broken part."
describles_original['cashew'] = "This is a photo of a cashew for anomaly detection, which should be without any damage, flaw, defect, scratch, hole or broken part."
describles_original['chewinggum'] = "This is a photo of a chewinggom for anomaly detection, which should be white, without any damage, flaw, defect, scratch, hole or broken part."
describles_original['fryum'] = "This is a photo of a fryum for anomaly detection on green background, which should be without any damage, flaw, defect, scratch, hole or broken part."
describles_original['macaroni1'] = "This is a photo of 4 macaronis for anomaly detection, which should be without any damage, flaw, defect, scratch, hole or broken part."
describles_original['macaroni2'] = "This is a photo of 4 macaronis for anomaly detection, which should be without any damage, flaw, defect, scratch, hole or broken part."
describles_original['pcb1'] = "This is a photo of pcb for anomaly detection, which should be without any damage, flaw, defect, scratch, hole or broken part."
describles_original['pcb2'] = "This is a photo of pcb for anomaly detection, which should be without any damage, flaw, defect, scratch, hole or broken part."
describles_original['pcb3'] = "This is a photo of pcb for anomaly detection, which should be without any damage, flaw, defect, scratch, hole or broken part."
describles_original['pcb4'] = "This is a photo of pcb for anomaly detection, which should be without any damage, flaw, defect, scratch, hole or broken part."
describles_original['pipe_fryum'] = "This is a photo of a pipe fryum for anomaly detection, which should be without any damage, flaw, defect, scratch, hole or broken part."

# Select which describles to use based on flag
active_describles = describles if command_args.use_prompt_adapter else describles_original
...
def predict_with_tta(input_text, image_path, normal_img_paths, describles_text, c_name):
    ...
    for i, (aug_path, reverse_type) in enumerate(augmentations):
        resp, anomaly_map = predict(describles_text + " Is there any anomaly in the image?",
                                     aug_path, normal_img_paths, 512, 0.01, 1.0, [], [])
...
for c_name in CLASS_NAMES:
    if command_args.use_hybrid:
        ...
        k = 2
        sel = 'original'
    print(f"\n[*] Processing {c_name} (k={k}, sel={sel}, hybrid={command_args.use_hybrid}, tta={command_args.use_tta}, prompt={command_args.use_prompt_adapter})")

    if sel == 'spread' and command_args.use_hybrid:
    ...
    for file_path in tqdm(file_paths[c_name], desc=c_name):
        if FEW_SHOT:
            use_tta_here = command_args.use_tta and (c_name not in NO_TTA_CLASSES)
            if use_tta_here:
                resp, anomaly_map = predict_with_tta(input_text, file_path, normal_img_path, active_describles[c_name], c_name)
            else:
                resp, anomaly_map = predict(active_describles[c_name] + ' ' + input_text, file_path, normal_img_path, 512, 0.01, 1.0, [], [])
        else:
            resp, anomaly_map = predict(active_describles[c_name] + ' ' + input_text, file_path, None, 512, 0.01, 1.0, [], [])
