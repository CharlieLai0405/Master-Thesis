...
def compute_feature_variance(all_paths, model_ref):
    """Compute feature-space diversity of normal images."""
    from model.ImageBind import data
    from model.ImageBind.models.imagebind_model import ModalityType
    batch_size = 16
    all_features = []
...
def predict_with_tta(input_text, image_path, normal_img_paths, describles_text):
    ...
    # Build augmentations: (image_or_path, reverse_type)
    augmentations = []
    ...
    # 4: 270 degree rotation (counterclockwise)
...
for c_name in CLASS_NAMES:
    ...
    # Auto spread detection based on feature diversity
    import glob
    all_normal_paths = sorted(glob.glob(os.path.join(normal_dir, "*.png")))
    diversity = compute_feature_variance(all_normal_paths, model)
    DIVERSITY_THRESHOLD = 0.02
    print(f"    [{c_name}] Feature diversity: {diversity:.4f} (threshold: {DIVERSITY_THRESHOLD})")
    if diversity > DIVERSITY_THRESHOLD:
        normal_img_paths = select_spread_references(all_normal_paths, max(k, 4), model)[:k]
        print(f"    [{c_name}] Using AUTO-SPREAD selection")
    else:
